...
def DrawCircle(radius, xShift = 0, yShift = 0, color='k',labelname="PicoBS",mark="b^"):
    '''
...
def PicoCircle(range1,range2,num=8):##range1/range2表示的是生成圆的圆心所在的范围,小圆半径默认100
    """
    ...
    while(len(x)<num+1):
        ...
        if range1<=np.sqrt(_x**2+_y**2)<=range2:
            x.append(_x)
            y.append(_y)
            if not Distance_points(x, y, 150):##保证pico基站之间的距离大于180
                x.pop()
                y.pop()
#         else:
...
#                 _y = R*sin(_t)
    for i in xrange(1,len(x)):
        DrawCircle(100, x[i], y[i], "g--", "PicoBS"+str(i), "k^")
    return x,y    #返回生成的 x,y list 的值
     

def RandomNum(num):
    """
            单位圆内生成  num 个随机分布在圆内的随机数
    """
...
def Draw(samples_num=100,R=500): #samples_num = 100  ### 样本数量,#R = 500 ##宏基站的大圆的半径
    """
    ...
    DrawCircle(R, 0, 0, "k-", "MacroBS","g^") #"k^"表示黑色三角圆心，画一个半径500的圆心位于原点宏基站圆
...
#     plt.ylim(-R-1,R+100)  ## 图片上y的范围
    ...
    plt.legend(loc="upper right",bbox_to_anchor=(1, 1),ncol=5, borderaxespad=0) ##显示图例 
    plt.show()  
    return R*x,R*y
