# -*-coding:utf-8 -*-
class CRC16(object):
    '''定义CRC16校验算法类,利用的是查表法，多项式是X^16+X^15+X^2+1。
    '''
    ...
    def createcrc(self,array):
        ...
        return (crchi<<8 | crclo)#返回类型为整形int
#     @staticmethod
    def createarray(self,array):
        '''createarray是直接返回生成crc的数组''' 
...
#     @staticmethod
    def calcrc(self,array):
        '''calcrc是校验数组是否符合crc16,返回0表示符合，1表示不符合''' 
        crchi = 0xff  
        crclo = 0xff
        lenarray = len(array)  
...
if __name__=="__main__":
    test = CRC16()
    aa = [0x7e,0x00,0x05,0x60,0x31,0x32,0x33]
    print type(test.createcrc(aa))
    print (hex(test.createcrc(aa)))
    print bin(test.createcrc(aa))
#     print chr(0x41)
